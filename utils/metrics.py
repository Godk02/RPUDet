...
def piou(box1, box2, xywh=True, PIoU=False,PIoU2=False,Lambda=1.3,eps=1e-7):
    ...
    ch = b1_y2.maximum(b2_y2) - b1_y1.minimum(b2_y1)  # convex heigt
    ...
    P=((dw1+dw2)/torch.abs(w2)+(dh1+dh2)/torch.abs(h2))/4 #原始


    # P = (0.5 * (dw1 + dw2) / torch.abs(w2) + 0.5 * torch.sqrt(dw1 + dw2) / torch.abs(w2) +
    #  0.5 * (dh1 + dh2) / torch.abs(h2) + 0.5 * torch.sqrt(dh1 + dh2) / torch.abs(h2)) / 4
