...
# 定义一个函数用于绘制矩形框
def draw_boxes(image, label_file, color_map=None, probability=0.9, change_people_to_pedestrian=False):
    ...
    for line in lines:
        ...
        # 如果需要将people类的一部分物体标记为pedestrian
        if change_people_to_pedestrian and class_id == 1:  # class_id 1 是 people
            if random.random() < 0.15:  # 30%的概率改变为 pedestrian（class_id 0）
                class_id = 0
...
# 主函数
def process_images(image_folder, label_folder, output_folder, color_map=None, probability=0.75, change_people_to_pedestrian=False):
    # 如果输出文件夹不存在，创建文件夹
...
output_folder = '/home/qk/data/VisDrone2019-DET-val/output3'  # 输出文件夹路径
...
# 设置概率为0.75，即每个物体有75%的概率会被绘制框
# change_people_to_pedestrian=True表示将30%的"people"类别标记为"pedestrian"
process_images(image_folder, label_folder, output_folder, color_map, probability=0.9, change_people_to_pedestrian=True)
