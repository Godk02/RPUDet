...
class yolov9_heatmap:
    def __init__(self, weight, device, method, layer, backward_type, conf_threshold, ratio, show_box, renormalize):
        ...
        method = eval(method)(model, target_layers)  # 移除 use_cuda 参数
    ...
    def draw_detections(self, box, color, name, img):
        ...
        thickness = max(1, int(img_height / 500))  # 框的粗细
        font_scale = max(0.5, img_height / 1000)  # 字体大小
        
        cv2.rectangle(img, (xmin, ymin), (xmax, ymax), tuple(int(x) for x in color), thickness)
        cv2.putText(img, str(name), (xmin, ymin - 5), cv2.FONT_HERSHEY_SIMPLEX, font_scale, tuple(int(x) for x in color), thickness, lineType=cv2.LINE_AA)
        return img
    ...
    def process(self, img_path, save_path):
        ...
        try:
            grayscale_cam = self.method(input_tensor=tensor, targets=[self.target])
...
if __name__ ==    '__main__':
    ...
    model(r'/home/qk/data/new_test/images', 'result_GRUdet-ldid-2')


# 省略了以下部分，为防止之前已发布的代码超长
